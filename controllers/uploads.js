...
const duploadFileCloudinary = async(req, res = response) => {
    const { tipo, id } = req.params;
    ...
    cloudinary.uploader.upload(file.tempFilePath, {folder: `uploads/${tipo}`}, async(err, result) => {
        if (err) {
            console.log(err);
            return res.status(500).json({
